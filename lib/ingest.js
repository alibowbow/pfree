...
export function classifySpot(get) {
  ...
  // 2) 혼합 + 기본요금 0 + 기본시간>0 → '최초 N분 무료' 유료장.
  //    무료로 분류하지 않는다(짧은 그레이스는 사실상 유료 — 사용자 정책).
  //    이후 단가 기준 1시간 요금이 저가 임계 이하면 low_cost 로만 노출.
  if (fee === '혼합' && baseFee === 0 && baseTime > 0) {
    const ut = fs && fs.unit_time, uf = fs && fs.unit_fee;
    const fhw = baseTime >= 60 ? 0 : (ut > 0 && uf > 0 ? Math.ceil((60 - baseTime) / ut) * uf : null);
    if (fhw != null && fhw <= LOW_COST_MAX_WON) {
      const info = [`최초 ${baseTime}분 무료`, ut > 0 && uf > 0 ? `이후 ${ut}분당 ${uf.toLocaleString('ko-KR')}원` : '']
        .filter(Boolean).join(' · ');
      return {
        category: 'low_cost',
        free_rules: mergeRules(remarkRules, [
          { day_type: '전일', fee_type: '최초N분무료', first_free_minutes: baseTime },
          { day_type: '전일', fee_type: '유료', fee_info: info },
        ]),
        fee_structure: fs, first_hour_won: fhw,
      };
    }
